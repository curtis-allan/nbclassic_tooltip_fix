(requirejs.specified("base/js/namespace")
  ...
  function get_cell_level(cell) {
    ...
    if (live_notebook) {
      ...
      var only_child_header = $(cell)
        .find(".inner_cell > .rendered_html > :only-child")
        .filter(":header");
  ...
  function update_heading_cell_status(cell) {
    ...
    if (cell_is_heading) {
      ...
      cell_elt.toggleClass(
        "collapsible_headings_ellipsis",
        params.show_ellipsis,
      );
      if (params.use_toggle_controls) {
        if (cht.length < 1) {
          ...
          if (live_notebook) {
            ...
            var only_child_header = cell_elt
              .find(".inner_cell > .rendered_html > :only-child")
              .filter(":header");
            clickable
              .add(only_child_header)
              .css("cursor", "pointer")
  ...
  function bracket_callback(evt) {
    ...
    if (bracket_level) {
      var bracket_cell = live_notebook
        ? bracket.closest(".cell").data("cell")
        : bracket.closest(".cell");
      ...
      switch (evt.type) {
        ...
        case "mouseenter":
        case "mouseleave":
          var in_section = get_jquery_bracket_section(header_cell).find(
            ".chb div[data-bracket-level=" + bracket_level + "]",
          );
  ...
  function toggle_heading(cell, set_collapsed, trigger_event) {
    if (is_heading(cell)) {
      ...
      update_collapsed_headings(
        params.show_section_brackets ? undefined : cell,
      );
      console.log(
        log_prefix,
        set_collapsed ? "collapsed" : "expanded",
        "cell",
        _find_cell_index(cell),
      );
      if (trigger_event !== false) {
        events.trigger(
          (set_collapsed ? "" : "un") + "collapse.CollapsibleHeading",
          { cell: cell },
        );
      }
    }
  ...
  //          var $cell = $(this.code_mirror.getWrapperElement()).closest(
  //            ".cell",
  //          );
    ...
    return new Promise(function (resolve, reject) {
      requirejs(
        ...
        function on_success(tooltip) {
          ...
          var action_down = kbm.actions.get(
      ...
      console.warn(
        log_prefix,
        "error patching Jupyter up/down actions:",
        reason,
      );
    });
  }
  ...
  function register_new_actions() {
    ...
    action_names.uncollapse = Jupyter.keyboard_manager.actions.register(
      {
        handler: function (env) {
          ...
          if (is_heading(cell)) {
            ...
            var ncells = env.notebook.ncells();
            for (
              var ii = env.notebook.find_cell_index(cell);
              ii < ncells;
              ii++
            ) {
    ...
    action_names.toggle = Jupyter.keyboard_manager.actions.register(
      {
        handler: function () {
          var heading_cell = find_header_cell(
          ...
          if (is_heading(heading_cell)) {
            ...
            Jupyter.notebook.select(
              Jupyter.notebook.find_cell_index(heading_cell),
            );
          }
        },
        ...
        handler: function () {
          ...
          for (var ii = 0; ii < cells.length; ii++) {
            if (is_heading(cells[ii])) {
              Jupyter.keyboard_manager.actions.call(
                action_names[
                  is_collapsed_heading(cells[ii])
                    ? "uncollapse_all"
                    : "collapse_all"
                ],
              );
  ...
  function insert_heading_cell(above) {
    ...
    new_cell.code_mirror.setSelection(
      { line: 0, ch: level + 1 },
      { line: 0, ch: level + 1 + new_text.length },
    );
  ...
  function set_collapsible_headings_options(options) {
    ...
    events[params.collapse_to_match_toc ? "on" : "off"](
      "collapse.Toc uncollapse.Toc",
      callback_toc_collapse,
    );
    // add css for indents
  ...
  function insert_menu_items() {
    $("#insert_menu")
      ...
      .append(
        make_action_menu_item(
          action_names.insert_above,
          "Insert Heading Above",
        ),
      )
      .append(
        make_action_menu_item(
          action_names.insert_below,
